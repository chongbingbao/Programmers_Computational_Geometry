...
namespace Miniball 
{
  ...
  Miniball<CoordAccessor>::Miniball (int d_, Pit begin, Pit end, CoordAccessor ca) 
    : mDimension (d_), 
      ...
      a (NULL)
  {	
    assert (mPointsBegin != mPointsEnd);
    create_arrays();

    // set initial center
    for (int j=0; j<mDimension; ++j) c[0][j] = nt0;
    mCurrentCenter = c[0];
  ...
  {
    c = new NT*[mDimension+1]; 
    v = new NT*[mDimension+1]; 
    a = new NT*[mDimension+1];
    for (int i=0; i<mDimension+1; ++i) {
      c[i] = new NT[mDimension];
      v[i] = new NT[mDimension];
      a[i] = new NT[mDimension];
    }
    sqr_r = new NT[mDimension+1];
    q0 = new NT[mDimension];
    z = new NT[mDimension+1];
    f = new NT[mDimension+1];
  } 
  ...
  Miniball<CoordAccessor>::relative_error (NT& subopt) const 
  {
    ...
    for (Pit i = mPointsBegin; i != mPointsEnd; ++i)
      if ((e = excess (i)) > max_e)
	max_e = e;
   
    subopt = suboptimality();
    assert (mCurrentSqrRadius > nt0 || max_e == nt0);
    return (mCurrentSqrRadius == nt0 ? nt0 : max_e / mCurrentSqrRadius);
  }
...
void Miniball<CoordAccessor>::mtf_mb (Sit n)
{
	...
	mSupportEnd = L.begin();
	if ((fsize) == mDimension+1) return;  

	// incremental construction
	for (Sit i = L.begin(); i != n;) 
	{// INV: (support_end - L.begin() == |S|-|B|)
		assert (std::distance (L.begin(), mSupportEnd) == ssize - fsize);
...
void Miniball<CoordAccessor>::pivot_mb (Pit n) 
{
	...
	do
	{
		old_sqr_r = mCurrentSqrRadius;
		sqr_r = mCurrentSqrRadius;

		pivot = mPointsBegin;
		max_e = nt0;
		for (k = mPointsBegin; k != n; ++k)
		{
			p = mCoordAccessor(k);
			e = -sqr_r;
			c = mCurrentCenter;
		...
		if (max_e > nt0)
		{
			...
			if (std::find(L.begin(), mSupportEnd, pivot) == mSupportEnd) 
			{
				assert (fsize == 0);
				if (push (pivot))
				{
	...
	} while (old_sqr_r < mCurrentSqrRadius);
}
...
{
	L.push_front(j);
	if (std::distance(L.begin(), mSupportEnd) == mDimension+2)
	  mSupportEnd--;
}
...
{
	Cit p = mCoordAccessor(pit);
	NT e  = -mCurrentSqrRadius;
	NT* c = mCurrentCenter;
...
bool Miniball<CoordAccessor>::push (Pit pit) 
{
	...
	else 
	{
		...
		if (z[fsize]<eps*mCurrentSqrRadius) 
		{
			return false;
		}
		// update c, sqr_r
	...
	mCurrentCenter = c[fsize];
	mCurrentSqrRadius = sqr_r[fsize];
	ssize = ++fsize;
	return true;
}
