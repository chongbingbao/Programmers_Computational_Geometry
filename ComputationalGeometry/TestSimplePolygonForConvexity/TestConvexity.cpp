...
*/
int GrahamPivot(Point2D* points,int numPoint,int& newNumPoint)
{
...
\brief	对比极角 p0-pivot-p1：
		...
		否则，p0与p1重叠，return 0。
*/
int GrahamCompare(const Point2D& p0 , const Point2D& p1, const Point2D& pivot)
{
	Real angle = (p0 - pivot).cross(p1 - pivot);
	if( EQUAL(angle,0) )
	{
		Real dif = pivot.distanceSquared(p0) - pivot.distanceSquared(p1);
		if( EQUAL(dif,0) )
			return 0;
...
*/
void GrahamHeapSort(Point2D * points, int numPoint , const Point2D& pivot)
{
...
*/
void	GrahamScanHull(const Point2D* pPoints,int numPoints,std::list<Point2D>& result)
{
	int i;
	Point2D* buffer = new Point2D[numPoints];
	...
	std::list<Point2D>::iterator last , current;
...
*/
bool InitConvex(const Point2D* point,int numPoint,int &mNumVertex,Point2D*& mVertex)
{
	if( numPoint<=2 )
		return false;
	int i;
	std::list<Point2D> result;
	...
	mVertex = new Point2D[mNumVertex];
...
\brief 判断多边形是否是凸多边形
*/
bool IsConvex(Point2D* vertex,int numVertex)
{
	...
	for(last = numVertex-2, current = numVertex-1, next = 0 ; next < numVertex ; last = current, current = next, next++)
	{
		...
		if( LESS(angle,0) )
			return false;
		else if(isOnLine && GREATER(angle,0))
			isOnLine = false;
	}
	if(!isOnLine)
		return true;
